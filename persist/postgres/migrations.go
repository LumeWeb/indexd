...
WHERE EXISTS (
	...
	func(ctx context.Context, tx *txn, log *zap.Logger) error {
		if _, err := tx.Exec(ctx, `ALTER TABLE hosts ADD COLUMN has_valid_addresses BOOLEAN NOT NULL DEFAULT FALSE`); err != nil {
			return err
		}
		const query = `
UPDATE hosts SET has_valid_addresses =
	EXISTS (
		SELECT 1 FROM host_addresses
		WHERE host_id = hosts.id
		  AND protocol = $1
	...
	AND EXISTS (
		SELECT 1 FROM host_addresses
		WHERE host_id = hosts.id AND protocol = $2
	)`
		if _, err := tx.Exec(ctx, query, networkProtocolQUIC, networkProtocolTCPSiaMux); err != nil {
			return err
		}
