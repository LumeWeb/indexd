...
func TestMigrationConsistency(t *testing.T) {
	...
	store, err := NewStore(ctx, ci, contracts.DefaultMaintenanceSettings, hosts.DefaultUsabilitySettings, log)
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	...
	baseline, err := NewStore(ctx, ci2, contracts.DefaultMaintenanceSettings, hosts.DefaultUsabilitySettings, log)
	if err != nil {
		t.Fatal(err)
	}
