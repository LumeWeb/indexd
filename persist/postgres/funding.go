...
// key that are due for funding, filtered by quota name. Only pools with at
// least one active account after the threshold are included.
func (s *Store) HostPoolsForFunding(hk types.PublicKey, quotaName string, threshold time.Time, limit int) ([]accounts.HostPool, error) {
	if limit < 0 {
		return nil, errors.New("limit can not be negative")
	} else if limit == 0 {
		return nil, nil
	...
	if err := s.transaction(func(ctx context.Context, tx *txn) error {
		...
		newPools, err := newHostPoolsForFunding(ctx, tx, hk, hostID, quotaName, threshold, remaining)
		if err != nil {
		...
		existingPools, err := existingHostPoolsForFunding(ctx, tx, hk, hostID, quotaName, threshold, remaining)
		if err != nil {
...
func (s *Store) InsertPoolAttachments(hk types.PublicKey, attachments []accounts.PendingAttachment) error {
	if len(attachments) > proto.MaxAccountBatchSize {
		return errors.New("too many attachments to insert")
...
func newHostPoolsForFunding(ctx context.Context, tx *txn, hk types.PublicKey, hostID int64, quotaName string, threshold time.Time, limit int) ([]accounts.HostPool, error) {
	pools := make([]accounts.HostPool, 0, limit)
...
AND EXISTS (SELECT 1 FROM accounts a WHERE a.connect_key_id = ack.id AND a.deleted_at IS NULL AND a.last_used >= $4)
LIMIT $2;`, hostID, limit, quotaName, threshold)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func existingHostPoolsForFunding(ctx context.Context, tx *txn, hk types.PublicKey, hostID int64, quotaName string, threshold time.Time, limit int) ([]accounts.HostPool, error) {
	pools := make([]accounts.HostPool, 0, limit)
