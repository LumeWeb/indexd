...
func TestHostManager(t *testing.T) {
	...
	// create host manager
	mgr, err := hosts.NewManager(&mockSyncer{peers: []*syncer.Peer{{}}}, &hosts.HostScanner{}, &mock.Locator{}, db, hosts.WithAnnouncementMaxAge(time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	defer mgr.Close()
...
func TestScanTimeout(t *testing.T) {
	runTest := func(t *testing.T, addr chain.NetAddress, scanner hosts.Scanner) {
		...
		// create host manager
		mgr, err := hosts.NewManager(&mockSyncer{peers: []*syncer.Peer{{}}}, scanner, &mock.Locator{}, db)
		if err != nil {
			t.Fatal(err)
		}
