...
func (s *testStore) setSectorsForCheck(t testing.TB, hk types.PublicKey, roots []types.Hash256) {
	...
	if _, err := s.Exec(context.Background(), "INSERT INTO stats_deltas (stat_name, stat_delta) VALUES ('num_unpinned_sectors', $1)", len(roots)); err != nil {
		t.Fatal(err)
	}
...
func (s *testStore) pinSectorToContract(t testing.TB, root types.Hash256, fcid types.FileContractID) {
	...
	if _, err := s.Exec(context.Background(), "INSERT INTO stats_deltas (stat_name, stat_delta) VALUES ('num_pinned_sectors', 1)"); err != nil {
		t.Fatal(err)
	}
	if _, err := s.Exec(context.Background(), "INSERT INTO stats_deltas (stat_name, stat_delta) VALUES ('num_unpinned_sectors', -1)"); err != nil {
		t.Fatal(err)
	}
